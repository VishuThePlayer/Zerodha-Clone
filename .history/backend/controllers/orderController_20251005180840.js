  ...
  try {
    ...
    const holdingsWithPrice = await Promise.all(
      holdings.map(async (stock) => {
        try {
          const priceRes = await axios.get(`http://localhost:5000/api/stock/live?q=${encodeURIComponent(stock.name)}`);
          const livePrice = priceRes.data?.data?.currentPrice?.NSE || stock.price || 0;
          console.
          return { ...stock._doc, price: livePrice }; // _doc is needed because Mongoose returns Document object
        } catch {
          return { ...stock._doc, price: stock.price || 0 }; // fallback to stored price
        }
      })
    );
