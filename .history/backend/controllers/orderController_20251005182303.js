  ...
  try {
    const holdings = await Holding.find({ user: req.userID }).populate("user", "username email");
      ...
      holdings.map(async (stock) => {
        try {
          axios.get(`http://localhost:5000/api/stock/live?q=${stock.name}`).then(async (res) => {
            let updatePricing = await Holding.findByIdAndUpdate(
            stock.id,
            { price: res.data?.data?stock.currentPrice?.NSE}
          )
          })
