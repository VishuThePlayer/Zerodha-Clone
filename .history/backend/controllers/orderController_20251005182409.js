...
exports.getAllHoldings = async (req, res) => {
  try {
    ...
    // Fetch and update all holdings prices in parallel
    const updatedHoldings = await Promise.all(
      holdings.map(async (stock) => {
        try {
          const response = await axios.get(`http://localhost:5000/api/stock/live?q=${stock.name}`);
          const newPrice = response.data?.data?.stock?.currentPrice?.NSE;

          if (newPrice) {
            await Holding.findByIdAndUpdate(stock._id, { price: newPrice });
            return { ...stock.toObject(), price: newPrice };
          }
          return stock;
        } catch (err) {
          console.error(`Failed to update price for ${stock.name}:`, err.message);
          return stock;
        }
      })
    );
