...
async function fetchNSEDataShareHolding(symbol) {
  console.log(`📍 [3/3] Fetching shareholding data for ${symbol}...`);
  const jar = await getOrCreateSession(symbol);
  
  const apiUrl = `https://www.nseindia.com/api/top-corp-info?symbol=${symbol}&market=equities&series=EQ`;
  const refererUrl = `https://www.nseindia.com/api/top-corp-info?symbol=${symbol}&market=equities&series=EQ`;
...
module.exports.getShareHolding = async (req, res) => {
  const symbol = req.params.symbol.toUpperCase();
  const startTime = Date.now();
  ...
  try {
    ...
    if (cached) {
      console.log(`✅ Cache hit for shareholding ${symbol} (${Date.now() - startTime}ms)`);
      return res.json({
        success: true,
        cached: true,
        data: cached,
    ...
    console.log(`\n🔍 Fetching shareholding for ${symbol}...`);
    const data = await fetchNSEDataShareHolding(symbol);

    // Cache the result
    cache.set(cacheKey, data);

    console.log(`✅ Fetched shareholding ${symbol} (${Date.now() - startTime}ms)\n`);
    res.json({
      success: true,
      cached: false,
      data: data,
    ...
    console.error(`❌ Error for shareholding ${symbol} (${Date.now() - startTime}ms):`);
    console.error(`   Message: ${error.message}`);
    console.error(`   Code: ${error.code}\n`);
    ...
    if (error.response?.status === 401 || error.response?.status === 403) {
      try {
        console.log("🔄 Retrying shareholding with fresh session...");
        sessionPool.jar = null;
        await delay(2000);
        const data = await fetchNSEDataShareHolding(symbol);
        cache.set(`shareholding-${symbol}`, data);
