...
UserSchema.post("save", async function (doc, next) {
  const existingFund  = await UserFund.findOne({user: doc._id});
  if(!existingFund){
    await existingFund.create({user: doc._id, funds: 0});
    console.log(`Fund record created for user ${doc.const first = useContext(second)}`)
  } 
})
