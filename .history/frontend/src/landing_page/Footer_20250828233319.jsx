import React from 'react'


function Footer() {
    return ( 
        <>
            <div className="container-sm">
                <div
                    class="row justify-content-center align-items-center g-2"
                >
                    <div class="col">
                        <img style={{width: "50%"}} src="https://zerodha.com/static/images/logo.svg" alt="" />
                        <div className="col mb-3 mt-4">
                            <h6 className='fs-5 fw-light'>Open demat account</h6>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Minor demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>NRI demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Commodity</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Dematerialisation</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Fund transfer</h5>
                        </div>
                    </div>
                    <div class="col gap-3">
                        <h4 className='fs-5 fw-medium'>Account</h4>
                        <div className="col mb-3 mt-4">
                            <h6 className='fs-5 fw-light'>Open demat account</h6>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Minor demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>NRI demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Commodity</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Dematerialisation</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Fund transfer</h5>
                        </div>
                    </div>
                    <div class="col">
                        <h4 className='fs-5 fw-medium'>Support</h4>
                        <div className="col mb-3 mt-4">
                            <h6 className='fs-5 fw-light'>Open demat account</h6>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Minor demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>NRI demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Commodity</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Dematerialisation</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Fund transfer</h5>
                        </div>
                    </div>
                    <div class="col">
                        <h4 className='fs-5 fw-medium'>Company</h4>
                        <div className="col mb-3 mt-4">
                            <h6 className='fs-5 fw-light'>Open demat account</h6>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Minor demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>NRI demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Commodity</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Dematerialisation</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Fund transfer</h5>
                        </div>
                    </div>
                    <div class="col">
                        <h4 className='fs-5 fw-medium'>Quick links</h4>
                        <div className="col mb-3 mt-4">
                            <h6 className='fs-5 fw-light'>Open demat account</h6>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Minor demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>NRI demat account</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Commodity</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Dematerialisation</h5>
                        </div>
                        <div className="col mb-3">
                            <h5 className='fs-5 fw-light'>Fund transfer</h5>
                        </div>
                    </div>
                </div>
                
            </div>
        </>
     );
}

export default Footer;