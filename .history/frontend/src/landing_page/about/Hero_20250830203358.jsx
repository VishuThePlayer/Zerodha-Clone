import React from 'react'
import {Link} from 'react-router-dom';

function Hero() {
  return (
    <>
      {/* Hero Section */}
      <div
        className="container d-flex flex-column justify-content-center align-items-center border-bottom border-secondary border-opacity-25 pb-3 mb-5"
        style={{ minHeight: '25vh' }}
      >
        <h1 className="text-center fw-medium fs-3">
          We pioneered the discount broking model in India. <br />
          Now, we are breaking ground with our technology.
        </h1>
      </div>

      {/* Two Columns Section */}
      <div className="container">
        <div style={{ fontSize: '20px' }} className="about_hero_section row justify-content-center text-start">
          <div className="about_hero_section_1 col-md-5 p-4">
            <p className="lh-lg mb-3">
              We kick-started operations on the 15th of August, 2010 with the goal of breaking all
              barriers that traders and investors face in India in terms of cost, support, and
              technology. We named the company Zerodha, a combination of Zero and &quot;Rodha&quot;,
              the Sanskrit word for barrier.
            </p>
            <p className="lh-lg mb-3">
              Today, our disruptive pricing models and in-house technology have made us the biggest
              stock broker in India.
            </p>
            <p className="lh-lg mb-3">
              Over 1.6+ crore clients place billions of orders every year through our powerful
              ecosystem of investment platforms, contributing over 15% of all Indian retail trading
              volumes.
            </p>
          </div>

          <div className="about_hero_section_1col-md-5 p-4">
            <p className="lh-lg mb-3">
              In addition, we run a number of popular open online educational and community
              initiatives to empower retail traders and investors.
            </p>
            <p className="lh-lg mb-3">
            <Link
                to="https://rainmatter.com/"
                className="text-decoration-none"
                target="_blank"  // optional: opens in new tab
            >
                Rainmatter
            </Link>
            , our fintech fund and incubator, has invested in several fintech startups
            with the goal of growing the Indian capital markets.
            </p>

            <p className="lh-lg mb-3">
              And yet, we are always up to something new every day. Catch up on the latest updates
              on our blog or see what the media is saying about us or learn more about our business
              and product philosophies.
            </p>
          </div>
        </div>
        <br />
        <br />
      </div>
    </>
  )
}

export default Hero
