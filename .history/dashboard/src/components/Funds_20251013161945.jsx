...
useEffect(() => {
  const fetchFunds = async () => {
    try {
      ...
      // ✅ Check success before setting state
      if (response.data.success) {
        setFunds(response.data.data);  // data = 0
        setIsFunds(true);
      } else {
        console.warn("Funds fetch unsuccessful");
      }
    } catch (error) {
        ...
        <div className="col">
          ...
          <div className="table">
            <div className="data">
              ...
              {isFunds ? (<p className="imp">{funds}</p>) : (<h1>Loading..</h1>)} 
            </div>
            <hr />
            <div className="data">
              <p>Used Balance</p>
