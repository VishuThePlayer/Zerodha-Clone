  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/stock/live/${encodeURIComponent(searchTerm)}`
      );
                      ...
                      <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                        ...
                        <Box>
                          <Typography variant="h5" fontWeight="bold">
                            {"N/A"||stock.info.companyName}
                          </Typography>
                          <Box display="flex" alignItems="center" gap={2} mt={1}>
                            <Chip label={"N/A"||stock.info.symbol} color="primary" size="small" variant="outlined" />
                            <Typography variant="body2" color="text.secondary">
                              • {"N/A"||stock.industryInfo.sector || stock.sector || "N/A"}
                            </Typography>
                          </Box>
                        </Box>

                        {/* Price Info */}
                        <Box textAlign="right" gap={3}>
                          <Typography variant="h4" fontWeight="bold">
                            {formatCurrency("N/A"|| stock.priceInfo?.lastPrice || stock.currentPrice)}
                          </Typography>
                          <Box
                            display="flex"
                            alignItems="center"
                            justifyContent="flex-end"
                            color={parseFloat("N/A"|| stock.percentChange) >= 0 ? "success.main" : "error.main"}
                            gap={0.5}
                          >
