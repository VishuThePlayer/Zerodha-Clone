...
export default function LiveStocks() {
  ...
  // 💰 Helpers
  const formatCurrency = (value) => (value ? `₹${parseFloat(value).toFixed(2)}` : "N/A");
        ...
        <Paper elevation={1} sx={{ p: 3, mt: 4, bgcolor: "grey.50" }}>
          ...
          <Box display="flex" flexWrap="wrap" gap={1}>
            {["Tata Steel", "JSW Steel", "TATASTEEL"].map((term) => (
              <Chip key={term} label={term} variant="outlined" onClick={() => setSearchTerm(term)} clickable />
            ))}
          </Box>
        </Paper>
      </Paper>
